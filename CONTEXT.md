...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **Sandcastle**                      | The TypeScript CLI tool that orchestrates an **agent** inside a **sandbox**                                                                    | "the tool", "the CLI", "RALPH"                                                                                           |
| **Sandbox**                         | The isolation boundary around a **workspace** — a container, VM, or similar environment that constrains the **agent**'s access                 | "container" (too specific), "Docker sandbox" (ambiguous with Claude's built-in feature), "workspace" (different concept) |
| **Sandbox provider**                | A pluggable implementation that creates and manages a **sandbox**, injected into `run()` via the `sandbox` option                              | "backend", "runtime", "sandbox factory"                                                                                  |
| **Bind-mount sandbox provider**     | A **sandbox provider** where the **host** filesystem is mounted directly into the environment                                                  | "local provider", "mount provider"                                                                                       |
| **Isolated sandbox provider**       | A **sandbox provider** where the environment has its own filesystem, requiring sync to move code in and commits out                            | "remote provider", "sync provider"                                                                                       |
| **No-sandbox provider**             | A **sandbox provider** where no container is created — the **agent** runs directly on the **host**                                             | "local provider", "none provider", "host provider"                                                                       |
| **Sandbox handle**                  | The object returned by a **sandbox provider**'s `create()` method, exposing `exec` and `close`                                                 | "sandbox instance", "sandbox connection"                                                                                 |
| **Workspace**                       | The directory where the **agent** runs — either the **host** working directory or a **worktree**                                               | "sandbox" (when referring to the directory), "working directory" (too generic)                                           |
| **Copy-to-workspace**               | An option on `run()` and `interactive()` that copies files into the **workspace** before execution                                             | "copyToSandbox" (old name — conflated workspace with sandbox)                                                            |
| **Branch strategy**                 | Configuration on a **sandbox provider** that controls how the agent's changes relate to branches, set at provider construction time            | "worktree mode" (old name), "branch mode"                                                                                |
| **Head (branch strategy)**          | A **branch strategy** where the **workspace** is the **host** working directory itself — no worktree, no branch indirection                    | `"none"` (old name), "direct"                                                                                            |
| **Merge-to-head (branch strategy)** | A **branch strategy** where Sandcastle creates a temporary branch, the agent works on it, and changes are merged back to HEAD                  | `"temp-branch"` (old name), "auto-branch"                                                                                |
...
| **Bundle/patch sync**               | A reusable utility for **isolated sandbox providers** that syncs repos via `git bundle` and extracts commits via `git format-patch` / `git am` | "sync service", "repo sync"                                                                                              |
...
| **Interactive arg collection**   | In `interactive()` mode, the behavior where Sandcastle prompts the user for `{{KEY}}` placeholders with no matching **prompt argument** | "arg prompting", "interactive fill"                                           |
...
| **Run log**          | A log file written to `.sandcastle/logs/` during a run session                                                              | "log file" (too generic), "output file"                                       |
| **Terminal mode**    | The display mode where Sandcastle renders an interactive UI in the terminal with spinners and styled status messages        | "stdout mode", "interactive mode", "CLI mode" (ambiguous with the CLI itself) |
| **Sandbox service**  | The internal Effect service interface that wraps the **sandbox handle** for use inside Sandcastle's Effect-based internals  | "adapter", "transport"                                                        |
| **Worktree**         | A git worktree created in `.sandcastle/worktrees/` on the **host**, used by the **merge-to-head** and **branch** strategies | "branch copy", "clone"                                                        |
